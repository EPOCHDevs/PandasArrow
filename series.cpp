...
namespace pd
{
    Series::Series(std::shared_ptr<arrow::Array> const& arr, bool isIndex,
                   std::string name): NDFrame<Series>(arr, nullptr, true),
    ...
    Series::Series(std::shared_ptr<arrow::Array> const& arr,
                   ...
                   std::string name,
                   bool skipIndex): NDFrame<Series>(arr, index, skipIndex),
            m_name(std::move(name)){}
    ...
    Series Series::operator-()  const{
        return ReturnSeriesOrThrowOnError(arrow::compute::Negate(
            m_array,
            arrow::compute::ArithmeticOptions{ false }));
    }
    ...
    {
        return ReturnScalarOrThrowOnError<double>(arrow::compute::Stddev(
            m_array,
    ...
    double Series::var(int ddof, bool skip_na)  const {
        return ReturnScalarOrThrowOnError<double>(arrow::compute::Variance(
            m_array,
            arrow::compute::VarianceOptions{ ddof, skip_na }));
    }
    ...
    Series Series::round_to_multiple(double multiple, arrow::compute::RoundMode roundMode) const {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::RoundToMultiple({ m_array }, opt));
    }

    Series Series::logb(int base)  const{
        return ReturnSeriesOrThrowOnError(
            arrow::compute::Logb({ m_array }, arrow::MakeScalar(base)));
    }
    ...
    {
        return ReturnSeriesOrThrowOnError(arrow::compute::CumulativeSum(
            m_array,
    ...
    {
        return ReturnSeriesOrThrowOnError(arrow::compute::CumulativeProduct(
            m_array,
    ...
    Resampler Series::resample(std::string const& rule,
                                  bool closed_right,
                                  bool label_right,
    ...
    Series Series::where(const Series & _filter) const {
        ...
        return ReturnSeriesOrThrowOnError(arrow::compute::CallFunction(
            "array_filter",
            { m_array, _filter.m_array },
            &opt));
    }
    ...
    Series StringLike::ascii_center(int64_t width, std::string const& padding) const
    {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("ascii_center", { m_array }, &opt));
    }
    ...
    Series StringLike::ascii_lpad(int64_t width, std::string const& padding) const
    {
        arrow::compute::PadOptions opt(width, padding);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("ascii_lpad", { m_array }, &opt));
    }
    ...
    Series StringLike::ascii_rpad(int64_t width, std::string const& padding) const
    {
        arrow::compute::PadOptions opt(width, padding);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("ascii_rpad", { m_array }, &opt));
    }
    ...
    Series StringLike::utf8_center(int64_t width, std::string const& padding) const
    {
        arrow::compute::PadOptions opt(width, padding);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("utf8_center", { m_array }, &opt));
    }
    ...
    Series StringLike::utf8_lpad(int64_t width, std::string const& padding) const
    {
        arrow::compute::PadOptions opt(width, padding);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("utf8_lpad", { m_array }, &opt));
    }
    ...
    Series StringLike::utf8_rpad(int64_t width, std::string const& padding) const
    {
        arrow::compute::PadOptions opt(width, padding);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("utf8_rpad", { m_array }, &opt));
    }
    ...
    Series StringLike::split_pattern(std::string const& pattern, int64_t max_splits, bool reverse) const {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("split_pattern", { m_array }, &opt));
    }
    ...
    Series StringLike::count_substring(const std::string &pattern, bool ignore_case) {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("count_substring", { m_array }, &opt));
    }
    ...
    Series StringLike::ends_with(const std::string &pattern, bool ignore_case) {
        arrow::compute::MatchSubstringOptions opt(pattern, ignore_case);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("ends_with", { m_array }, &opt));
    }

    Series StringLike::find_substring(const std::string &pattern, bool ignore_case) {
        arrow::compute::MatchSubstringOptions opt(pattern, ignore_case);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("find_substring", { m_array }, &opt));
    }

    Series StringLike::match_like(const std::string &pattern, bool ignore_case) {
        arrow::compute::MatchSubstringOptions opt(pattern, ignore_case);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("match_like", { m_array }, &opt));
    }
    ...
    Series StringLike::match_substring(const std::string &pattern, bool ignore_case) {
        arrow::compute::MatchSubstringOptions opt(pattern, ignore_case);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("match_substring", { m_array }, &opt));
    }

    Series StringLike::starts_with(const std::string &pattern, bool ignore_case) {
        arrow::compute::MatchSubstringOptions opt(pattern, ignore_case);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("starts_with", { m_array }, &opt));
    }
    ...
    Series StringLike::index_in(const Series &value_set, bool skip_nulls) {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("index_in", { m_array }, &opt));
    }

    Series StringLike::is_in(const Series &value_set, bool skip_nulls) {
        arrow::compute::SetLookupOptions opt(value_set.array(), skip_nulls);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("is_in", { m_array }, &opt));
    }
    ...
    Series DateTimeLike::ceil(int multiple, arrow::compute::CalendarUnit unit, bool week_starts_monday,
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("ceil_temporal", { m_array }, &opt));
    }
    ...
    Series DateTimeLike::floor(int multiple, arrow::compute::CalendarUnit unit, bool week_starts_monday,
        ...
        arrow::compute::RoundTemporalOptions opt(multiple, unit, week_starts_monday,
                                                 ceil_is_strictly_greater, calendar_based_origin);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("floor_temporal", { m_array }, &opt));
    }
    ...
    Series DateTimeLike::round(int multiple, arrow::compute::CalendarUnit unit, bool week_starts_monday,
        ...
        arrow::compute::RoundTemporalOptions opt(multiple, unit, week_starts_monday,
                                                 ceil_is_strictly_greater, calendar_based_origin);
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("round_temporal", { m_array }, &opt));
    }

    Series Series::cast(const std::shared_ptr<arrow::DataType> &dt, bool safe) const
    {
        return ReturnSeriesOrThrowOnError(arrow::compute::Cast(
            m_array,
            dt,
    ...
    Series Series::strftime(const std::string &format, std::string const& locale)  const {
        return ReturnSeriesOrThrowOnError(arrow::compute::Strftime(
            m_array,
    ...
    Series Series::strptime(const std::string &format,
        ...
        return ReturnSeriesOrThrowOnError(arrow::compute::Strptime(
            m_array,
    ...
    {
        return ReturnSeriesOrThrowOnError(arrow::compute::Shift(
            m_array,
    ...
    {
        return ReturnSeriesOrThrowOnError(
            arrow::compute::PctChange(m_array, periods));
    }
    ...
    GenericFunctionSeriesReturnRename(ffill, fill_null_forward, Series)
        ...
        {
            return ReturnSeriesOrThrowOnError(arrow::compute::ReplaceWithMask(
                m_array,
                cond.m_array,
                other.m_array));
        }
    ...
    Series Series::intersection(Series const& other) const
    {
        ...
        auto intersection_array = ReturnOrThrowOnFailure(arrow::compute::Take(
            m_array,
    ...
    Series Series::union_(const Series &other) const
    {
        ...
        auto concatenated_arrays = pd::ReturnOrThrowOnFailure(
            arrow::Concatenate({ m_array, other.m_array }));

        auto new_series = Series(
            pd::ReturnOrThrowOnFailure(
                arrow::compute::Unique(concatenated_arrays)),
            nullptr,
            m_name,
            true);
    ...
    double Series::cov(const Series &s2)  const{
        return ReturnScalarOrThrowOnError<double>(arrow::compute::Covariance(
            m_array,
            s2.m_array,
    ...
    double Series::corr(const Series &s2, CorrelationType method) const
    {
        switch (method)
        {
            case CorrelationType::Pearson:
                return ReturnScalarOrThrowOnError<double>(arrow::Correlation(
                    m_array,
                    s2.m_array,
                    arrow::compute::VarianceOptions(1)));
                ...
                int min_periods) const
    {
        ...
        auto casted =
            ReturnOrThrowOnFailure(arrow::compute::Cast(m_array, DoubleTypePtr))
                .make_array();
        auto doubleArray =
    ...
    Series Series::operator[](const StringSlice & slicer) const {
        if(m_index->type_id() == arrow::Type::STRING)
        {
            ...
            {
                start =
                    ReturnScalarOrThrowOnError<int64_t>(arrow::compute::Index(
                        m_index,
                        arrow::compute::IndexOptions{
                            arrow::MakeScalar(slicer.start.value()) }));
                if(start == -1)
                {
            ...
            if(slicer.end) {
                end = ReturnScalarOrThrowOnError<int64_t>(arrow::compute::Index(
                    m_index,
                    arrow::compute::IndexOptions{
                        arrow::MakeScalar(slicer.end.value()) }));
                if(end == -1)
                {
    ...
    Series Series::if_else(const Series &truth_values, const Series &other) const {
        return ReturnSeriesOrThrowOnError(arrow::compute::IfElse(
            truth_values.m_array,
            m_array,
            other.m_array));
    }
                               ...
                               std::optional< std::unordered_map<int64_t, int64_t>>  indexer) const
    {
        ...
        auto new_idx_int = pd::ReturnOrThrowOnFailure(arrow::compute::Cast(newIndex, {arrow::int64()})).array_as<arrow::Int64Array>();
        ...
        if(not indexer)
        {
            ...
            auto idx_int = pd::ReturnOrThrowOnFailure(arrow::compute::Cast(m_index, {arrow::int64()})).array_as<arrow::Int64Array>();
                                    ...
                                    std::optional< std::unordered_map<int64_t, int64_t>>  indexer) const
    {
        ...
        auto new_idx_int =
            pd::ReturnOrThrowOnFailure(
                arrow::compute::Cast(newIndex, { arrow::int64() }))
                .array_as<arrow::Int64Array>();
        // Create a new values array builder for the reindexed series
        ...
        if (not indexer)
        {
            ...
            auto idx_int =
                pd::ReturnOrThrowOnFailure(
                    arrow::compute::Cast(m_index, { arrow::int64() }))
                    .array_as<arrow::Int64Array>();
            for (int i = 0; i < m_index->length(); i++)
            {
                indexer->insert_or_assign(idx_int->Value(i), i);
        ...
        auto newValuesBuilder =
            pd::ReturnOrThrowOnFailure(arrow::MakeBuilder(m_array->type()));
    ...
    Series DateTimeLike::week(bool week_starts_monday,bool count_from_zero, bool first_week_is_fully_in_year) const {
        ...
        return ReturnSeriesOrThrowOnError(
            arrow::compute::CallFunction("week", { m_array }, &opt));
    }
