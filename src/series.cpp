...
#define Aggregation(name, ReturnT) \
    ...
    { \
        ...
        return ReturnScalarOrThrowOnError<ReturnT>(arrow::compute::CallFunction(#name, { m_array }, &opt)); \
    }
...
GenericFunctionScalarReturn(min) GenericFunctionScalarReturn(max) GenericFunctionScalarReturn(product)
    GenericFunctionScalarReturn(sum)

        Scalar Series::agg(std::string const& name, bool skip_null) const
{
    arrow::compute::ScalarAggregateOptions opt{ skip_null };
...
Scalar Series::quantile(double q) const
{
    ...
    if (result.ok())
    {
        return pd::ReturnOrThrowOnFailure(result->make_array()->GetScalar(0));
    }
    else
    {
        throw std::runtime_error(result.status().ToString());
...
Scalar Series::tdigest(double q) const
{
    ...
    if (result.ok())
    {
        return ReturnOrThrowOnFailure(result->make_array()->GetScalar(0));
    }
    else
    {
        throw std::runtime_error(result.status().ToString());
...
{
    auto result = arrow::compute::MinMax(m_array, arrow::compute::ScalarAggregateOptions{ skip_null });
    if (result.ok())
    {
...
Aggregation(all, bool) Aggregation(any, bool) Aggregation(mean, double)
                ...
                double Series::std(int ddof, bool skip_na) const
{
    return ReturnScalarOrThrowOnError<double>(
        arrow::compute::Stddev(m_array, arrow::compute::VarianceOptions{ ddof, skip_na }));
}
...
int64_t Series::index(Scalar const& search) const
{
    ...
    {
        auto result = arrow::compute::Index(m_array, arrow::compute::IndexOptions{ search.value() });
        if (result.ok())
        {
...
{
    arrow::compute::ScalarAggregateOptions opt{ skip_null };
    auto result = arrow::compute::CallFunction("approximate_median", { m_array }, &opt);
    if (result.ok())
    {
...
{
    return ReturnScalarOrThrowOnError<double>(
        arrow::compute::Variance(m_array, arrow::compute::VarianceOptions{ ddof, skip_na }));
}
...
GenericFunctionSeriesReturn(abs) GenericFunctionSeriesReturn(sqrt) GenericFunctionSeriesReturn(sign)
    GenericFunctionSeriesReturn(ln) GenericFunctionSeriesReturn(log10) GenericFunctionSeriesReturn(log1p)
        GenericFunctionSeriesReturn(log2) GenericFunctionSeriesReturn(cos) GenericFunctionSeriesReturn(sin)
            GenericFunctionSeriesReturn(tan) GenericFunctionSeriesReturn(ceil) GenericFunctionSeriesReturn(floor)
                GenericFunctionSeriesReturn(round) GenericFunctionSeriesReturn(trunc)
...
Series Series::take(const Series& x) const
{
    ...
    {
        auto arr = ReturnOrThrowOnFailure(arrow::compute::CallFunction("array_filter", { m_array, x.m_array })).make_array();
        auto idx = ReturnOrThrowOnFailure(arrow::compute::CallFunction("array_filter", { m_index, x.m_array })).make_array();
        return {arr, idx};
    }
    else
    {
        auto arr = ReturnOrThrowOnFailure(arrow::compute::CallFunction("array_take", { m_array, x.m_array })).make_array();
        auto idx = ReturnOrThrowOnFailure(arrow::compute::CallFunction("array_take", { m_index, x.m_array })).make_array();
        return {arr, idx};
    }
}

GenericFunctionSeriesReturnString(ascii_capitalize) GenericFunctionSeriesReturnString(
    ascii_lower) GenericFunctionSeriesReturnString(ascii_reverse) GenericFunctionSeriesReturnString(ascii_swapcase)
    GenericFunctionSeriesReturnString(ascii_title) GenericFunctionSeriesReturnString(ascii_upper)

        GenericFunctionScalarReturnString(binary_length) GenericFunctionSeriesReturnString(
            binary_repeat) GenericFunctionSeriesReturnString(binary_reverse)

            GenericFunctionSeriesReturnString(utf8_capitalize) GenericFunctionSeriesReturnString(utf8_length)
                GenericFunctionSeriesReturnString(utf8_lower) GenericFunctionSeriesReturnString(utf8_reverse)
                    GenericFunctionSeriesReturnString(utf8_swapcase) GenericFunctionSeriesReturnString(
                        utf8_title) GenericFunctionSeriesReturnString(utf8_upper)

                        GenericFunctionSeriesReturnString(ascii_is_alnum) GenericFunctionSeriesReturnString(
                            ascii_is_alpha) GenericFunctionSeriesReturnString(ascii_is_decimal)
                            GenericFunctionSeriesReturnString(ascii_is_lower) GenericFunctionSeriesReturnString(
                                utf8_is_alnum) GenericFunctionSeriesReturnString(utf8_is_alpha)
                                GenericFunctionSeriesReturnString(utf8_is_decimal) GenericFunctionSeriesReturnString(
                                    utf8_is_digit) GenericFunctionSeriesReturnString(utf8_is_lower)
                                    GenericFunctionSeriesReturnString(
                                        utf8_is_numeric) GenericFunctionSeriesReturnString(utf8_is_printable)
                                        GenericFunctionSeriesReturnString(
                                            utf8_is_space) GenericFunctionSeriesReturnString(utf8_is_upper)

                                            GenericFunctionSeriesReturnString(
                                                ascii_is_title) GenericFunctionSeriesReturnString(utf8_is_title)
...
Series Series::ewm(EWMAgg agg, double value, EWMAlphaType type, bool adjust, bool ignore_na, int min_periods, bool bias)
    const
{
    ...
    switch (agg)
    {
        ...
        case EWMAgg::StdDev:
            ...
            {
                std::ranges::transform(output, output.begin(), [](auto const& x) { return x < 0 ? 0 : std::sqrt(x); });
            }
            break;
    }
...
Series Series::operator[](const StringSlice& slicer) const
{
    if (m_index->type_id() == arrow::Type::STRING)
    {
        ...
        if (slicer.start)
        {
            start = ReturnScalarOrThrowOnError<int64_t>(arrow::compute::Index(
                m_index,
        ...
        if (slicer.end)
        {
                ...
                arrow::compute::Index(m_index, arrow::compute::IndexOptions{ arrow::MakeScalar(slicer.end.value()) }));
            if (end == -1)
            {
            ...
            if (mean_x_check)
            {
                ...
                if (is_observation or not ignore_na)
                {
                    ...
                    if (is_observation)
                    {
                        ...
                        cov = ((old_wt *
                                (cov + ((old_mean_x - mean_x_check.value()) * (old_mean_y - mean_y_check.value())))) +
...
GenericFunctionSeriesReturn(is_finite) GenericFunctionSeriesReturn(is_valid) GenericFunctionSeriesReturn(is_null)
...
GenericFunctionSeriesReturnDateTimeLike(day) GenericFunctionSeriesReturnDateTimeLike(day_of_week)
    GenericFunctionSeriesReturnDateTimeLike(day_of_year) GenericFunctionSeriesReturnDateTimeLike(hour)
        GenericFunctionSeriesReturnDateTimeLike(is_dst) GenericFunctionSeriesReturnDateTimeLike(iso_week)
            GenericFunctionSeriesReturnDateTimeLike(iso_year) GenericFunctionSeriesReturnDateTimeLike(is_leap_year)
                GenericFunctionSeriesReturnDateTimeLike(microsecond)
                    GenericFunctionSeriesReturnDateTimeLike(millisecond) GenericFunctionSeriesReturnDateTimeLike(month)
...
{
    return ReturnSeriesOrThrowOnError(arrow::compute::CallFunction("microseconds_between", { m_array, other.m_array }));
}

Series DateTimeLike::milliseconds_between(const Series& other) const
{
    return ReturnSeriesOrThrowOnError(arrow::compute::CallFunction("milliseconds_between", { m_array, other.m_array }));
}
...
{
    return ReturnSeriesOrThrowOnError(arrow::compute::CallFunction("nanoseconds_between", { m_array, other.m_array }));
}
