...
struct DateTimeLike : public Series
{
    ...
    Series week(bool week_starts_monday = true, bool count_from_zero = false, bool first_week_is_fully_in_year = false)
        const;
    Series year() const;
...
private:
    ...
    DateTimeLike(const std::shared_ptr<arrow::Array>& array) : Series(array, nullptr, "", true)
    {
    }
};
} // namespace pd