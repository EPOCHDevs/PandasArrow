...
Concatenator::Concatenator(
    const std::vector<pd::DataFrame>& objs,
    JoinType join,
    bool ignore_index,
    bool sort):sort(sort), ignore_index(ignore_index)
{
    if (objs.empty())
    {
...
arrow::ArrayVector Concatenator::makeJoinIndexes(
    std::vector<pd::DataFrame> const& objs,
    AxisType concat_axis)
{
    auto selectIndex = concat_axis == AxisType::Columns ?
...
ArrayPtr Concatenator::makeConcatIndex(
    std::vector<pd::DataFrame> const& objs,
    AxisType axis,
    bool ignoreIndex)
{
    auto selectIndex = axis == AxisType::Columns ?
        [](pd::DataFrame const& obj) { return obj.fieldArray(); } :
        [](pd::DataFrame const& obj) { return obj.indexArray(); };
    ...
    if (ignoreIndex)
    {
        ...
        ::uint64_t totalSize =
            std::accumulate(objs.begin(), objs.end(), 0UL, getSize);
...
ArrayPtr Concatenator::mergeIndexes(
...
std::unordered_map<std::string, std::pair<std::vector<int>, std::shared_ptr<arrow::DataType>> >
    Concatenator::resolveDuplicateFieldName(std::vector<pd::DataFrame> const& objs)
{
    std::unordered_map<
        std::string,
        std::pair<std::vector<int>, std::shared_ptr<arrow::DataType>>>
        result;
    ...
    for (auto const& obj : objs)
    {
        for (auto const& field : obj.array()->schema()->fields())
        {
            ...
            {
                result[fieldName] =
                    std::pair{ std::vector{ i }, field->type() };
            }
            else
            {
                result[fieldName].first.push_back(i);
                result[fieldName].second =
                    promoteTypes({ result[fieldName].second, field->type() });
            }
        }
        i++;
    }
    ...
    bool sort)
{
        ...
        for (auto const& [field, value] : newDataTypes)
        {
            if (value.first.size() > 1)
            {
                ...
                auto casted = arrow::compute::Cast(
                                  recordBatch->column(field_index),
                                  new_type)
                                  ->make_array();
                objs.at(idx) = DataFrame{ pd::ReturnOrThrowOnFailure(
                                              recordBatch->SetColumn(
                                                  field_index,
                                                  arrow::field(field, new_type),
                                                  casted)),
        ...
        std::ranges::transform(
            objs,
            tables.begin(),
            [&unique_index](auto const& obj)
            { return obj.toTable(unique_index); });
        ...
        auto table = pd::ReturnOrThrowOnFailure(
            arrow::ConcatenateTables(tables, options));
        ...
        merged_rb =
            pd::ReturnOrThrowOnFailure(merged_rb->RemoveColumn(index_column_i));
...
{
        auto newIndexes =
            mergeIndexes(makeJoinIndexes(objs, AxisType::Columns), intersect);
        ...
        tbb::parallel_for(
            ...
            [&](size_t i)
            {
                ...
                std::ranges::copy(
                    obj.array()->schema()->fields(),
                    fieldVector.begin() + index_offset[i]);
                std::ranges::copy(
                    obj.array()->column_data(),
                    arrayVectors.begin() + index_offset[i]);
            });
        ...
        if (ignore_index)
        {
            ...
            {
                fieldVector.at(i) =
                    arrow::field(std::to_string(i), fieldVector.at(i)->type());
            }
        }
