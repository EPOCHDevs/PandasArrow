...
struct StringLike : public Series
{
    ...
    Series replace_substring(
        std::string const& pattern,
        std::string const& replacement,
        int64_t const& max_replacements = -1) const;
    Series replace_substring_regex(
        std::string const& pattern,
        std::string const& replacement,
        int64_t const& max_replacements = -1) const;
    ...
    Series utf8_replace_slice(int64_t start, int64_t stop, std::string const& replacement) const;
    ...
    Series utf8_slice_codeunits(int64_t start, int64_t stop = std::numeric_limits<int64_t>::max(), int64_t step = 1)
        const;
...
private:
    ...
    StringLike(const std::shared_ptr<arrow::Array>& array) : Series(array, nullptr, "", true)
    {
    }
};
} // namespace pd