...
struct Add
{
    ...
    static constexpr enable_if_floating_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status*)
    {
        return left + right;
    }

    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_unsigned_integer_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status*)
    {
        return left + right;
    }

    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_signed_integer_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status*)
    {
...
{
    template<typename T, typename Arg0, typename Arg1>
    static enable_if_integer_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status* st)
    {
...
struct AddTimeDuration
{
    ...
    {
        T result = arrow::internal::SafeSignedAdd(static_cast<T>(left), static_cast<T>(right));
        if (result < 0 || multiple <= result)
        {
            *st = Status::Invalid(result, " is not within the acceptable range of ", "[0, ", multiple, ") s");
        }
        return result;
    }
};
...
struct AddTimeDurationChecked
{
    ...
    {
        T result = 0;
        if (ARROW_PREDICT_FALSE(AddWithOverflow(static_cast<T>(left), static_cast<T>(right), &result)))
        {
            *st = Status::Invalid("overflow");
        }
        if (result < 0 || multiple <= result)
...
struct AbsoluteValue
{
    ...
    static constexpr enable_if_floating_value<Arg, T> Call(KernelContext*, Arg arg, Status*)
    {
        return std::fabs(arg);
    }

    template<typename T, typename Arg>
    static constexpr enable_if_unsigned_integer_value<Arg, T> Call(KernelContext*, Arg arg, Status*)
    {
        return arg;
    }

    template<typename T, typename Arg>
    static constexpr enable_if_signed_integer_value<Arg, T> Call(KernelContext*, Arg arg, Status* st)
    {
        return (arg < 0) ? arrow::internal::SafeSignedNegate(arg) : arg;
    }

    template<typename T, typename Arg>
    static constexpr enable_if_decimal_value<Arg, T> Call(KernelContext*, Arg arg, Status*)
    {
        return arg.Abs();
    }
};
...
{
    template<typename T, typename Arg>
    static enable_if_signed_integer_value<Arg, T> Call(KernelContext*, Arg arg, Status* st)
    {
    ...
    template<typename T, typename Arg>
    static enable_if_unsigned_integer_value<Arg, T> Call(KernelContext* ctx, Arg arg, Status* st)
    {
        static_assert(std::is_same<T, Arg>::value, "");
        return arg;
    }
    ...
    template<typename T, typename Arg>
    static constexpr enable_if_floating_value<Arg, T> Call(KernelContext*, Arg arg, Status* st)
    {
        static_assert(std::is_same<T, Arg>::value, "");
        return std::fabs(arg);
    }
...
struct Multiply
{
    ...
    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_floating_value<T> Call(KernelContext*, T left, T right, Status*)
    {
        return left * right;
    }

    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_t<is_unsigned_integer_value<T>::value && !std::is_same<T, uint16_t>::value, T> Call(
        KernelContext*,
        T left,
        T right,
        Status*)
    ...
    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_t<is_signed_integer_value<T>::value && !std::is_same<T, int16_t>::value, T> Call(
        KernelContext*,
        T left,
        T right,
        Status*)
    ...
    // well defined.
    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_same<T, int16_t, T> Call(KernelContext*, int16_t left, int16_t right, Status*)
    {
        return static_cast<uint32_t>(left) * static_cast<uint32_t>(right);
    }
    template<typename T, typename Arg0, typename Arg1>
    static constexpr enable_if_same<T, uint16_t, T> Call(KernelContext*, uint16_t left, uint16_t right, Status*)
    {
        return static_cast<uint32_t>(left) * static_cast<uint32_t>(right);
    }
...
struct Divide
{
    ...
    template<typename T, typename Arg0, typename Arg1>
    static enable_if_decimal_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status* st)
    {
        if (right == Arg1())
        {
            *st = Status::Invalid("Divide by zero");
...
struct DivideChecked
{
    ...
    template<typename T, typename Arg0, typename Arg1>
    static enable_if_floating_value<T> Call(KernelContext*, Arg0 left, Arg1 right, Status* st)
    {
        static_assert(std::is_same<T, Arg0>::value && std::is_same<T, Arg1>::value, "");
    ...
    template<typename T, typename Arg0, typename Arg1>
    static enable_if_decimal_value<T> Call(KernelContext* ctx, Arg0 left, Arg1 right, Status* st)
    {
...
struct Negate
{
    ...
    template<typename T, typename Arg>
    static constexpr enable_if_unsigned_integer_value<T> Call(KernelContext*, Arg arg, Status*)
    {
        return ~arg + 1;
    }

    template<typename T, typename Arg>
    static constexpr enable_if_signed_integer_value<T> Call(KernelContext*, Arg arg, Status*)
    {
...
{
    template<typename T, typename Arg0, typename Arg1>
    static enable_if_integer_value<T> Call(KernelContext*, Arg0 base, Arg1 exp, Status* st)
    {
        if (exp < 0)
        {
            *st = Status::Invalid("integers to negative integer powers are not allowed");
        ...
        bool overflow = false;
        uint64_t bitmask = 1ULL << (63 - bit_util::CountLeadingZeros(static_cast<uint64_t>(exp)));
        T pow = 1;
        while (bitmask)
        {
...
struct Sign
{
    ...
    template<typename T, typename Arg>
    static constexpr enable_if_signed_integer_value<Arg, T> Call(KernelContext*, Arg arg, Status*)
    {
