...
struct OptionsWrapper : public KernelState
{
    ...
    static const OptionsType& Get(KernelContext* ctx)
    {
...
struct KernelStateFromFunctionOptions : public KernelState
{
    ...
    {
        return ::arrow::internal::checked_cast<const KernelStateFromFunctionOptions&>(state).state;
    }

    static const StateType& Get(KernelContext* ctx)
    {
        return Get(*ctx->state());
    }
...
template<typename Type>
struct GetViewType<Type, enable_if_t<is_base_binary_type<Type>::value || is_fixed_size_binary_type<Type>::value>>
{
...
template<typename T>
using is_signed_integer_value = std::integral_constant<bool, std::is_integral<T>::value && std::is_signed<T>::value>;

template<typename T, typename R = T>
using enable_if_signed_integer_value = enable_if_t<is_signed_integer_value<T>::value, R>;

template<typename T, typename R = T>
using enable_if_unsigned_integer_value = enable_if_t<is_unsigned_integer_value<T>::value, R>;

template<typename T, typename R = T>
using enable_if_integer_value =
    enable_if_t<is_signed_integer_value<T>::value || is_unsigned_integer_value<T>::value, R>;
...
using enable_if_decimal_value =
    enable_if_t<std::is_same<Decimal128, T>::value || std::is_same<Decimal256, T>::value, R>;

// ----------------------------------------------------------------------
// Iteration / value access utilities

template<typename T, typename R = void>
using enable_if_c_number_or_decimal =
    enable_if_t<(has_c_type<T>::value && !is_boolean_type<T>::value) || is_decimal_type<T>::value, R>;
...
struct ArrayIterator<Type, enable_if_boolean<Type>>
{
    ...
    explicit ArrayIterator(const ArraySpan& arr) : reader(arr.buffers[1].data, arr.offset, arr.length)
    {
    }
    bool operator()()
    {
...
struct ArrayIterator<Type, enable_if_base_binary<Type>>
{
    ...
    explicit ArrayIterator(const ArraySpan& arr)
        : arr(arr), offsets(reinterpret_cast<const offset_type*>(arr.buffers[1].data) + arr.offset),
          cur_offset(offsets[0]), data(reinterpret_cast<const char*>(arr.buffers[2].data)), position(0)
    {
    }
...
struct ArrayIterator<FixedSizeBinaryType>
{
    ...
    explicit ArrayIterator(const ArraySpan& arr)
        : arr(arr), data(reinterpret_cast<const char*>(arr.buffers[1].data)), width(arr.type->byte_width()),
          position(arr.offset)
    {
    }
...
struct UnboxScalar<Type, enable_if_has_c_type<Type>>
{
    ...
    {
        std::string_view view = checked_cast<const ::arrow::internal::PrimitiveScalarBase&>(val).view();
...
struct BoxScalar<Type, enable_if_has_c_type<Type>>
{
    ...
    static void Box(T val, Scalar* out)
    {
        ...
        T* mutable_data =
            reinterpret_cast<T*>(checked_cast<::arrow::internal::PrimitiveScalarBase*>(out)->mutable_data());
        *mutable_data = val;
    }
};
...
struct BoxScalar<Decimal128Type>
{
    ...
    static void Box(T val, Scalar* out)
    {
        checked_cast<ScalarType*>(out)->value = val;
    }
};
...
static typename ::arrow::internal::call_traits::enable_if_return<VisitFunc, void>::type VisitArrayValuesInline(
    const ArraySpan& arr,
    ...
    NullFunc&& null_func)
{
    VisitArraySpanInline<T>(
        arr,
        [&](typename GetViewType<T>::PhysicalType v) { valid_func(GetViewType<T>::LogicalValue(std::move(v))); },
        std::forward<NullFunc>(null_func));
}

template<typename T, typename VisitFunc, typename NullFunc>
static typename ::arrow::internal::call_traits::enable_if_return<VisitFunc, Status>::type VisitArrayValuesInline(
    const ArraySpan& arr,
    VisitFunc&& valid_func,
    NullFunc&& null_func)
{
    return VisitArraySpanInline<T>(
        arr,
        [&](typename GetViewType<T>::PhysicalType v) { return valid_func(GetViewType<T>::LogicalValue(std::move(v))); },
        std::forward<NullFunc>(null_func));
}
...
static void VisitTwoArrayValuesInline(
    const ArraySpan& arr0,
    ...
    NullFunc&& null_func)
{
    ...
    { valid_func(GetViewType<Arg0Type>::LogicalValue(arr0_it()), GetViewType<Arg1Type>::LogicalValue(arr1_it())); };
    auto visit_null = [&]()
    {
        arr0_it();
        arr1_it();
...
struct OutputAdapter<Type, enable_if_boolean<Type>>
{
    ...
    {
        GenerateBitsUnrolled(out->buffers[1].data, out->offset, out->length, std::forward<Generator>(generator));
        return Status::OK();
    }
};
...
struct ScalarUnary
{
    ...
    static Status Exec(KernelContext* ctx, const ExecSpan& batch, ExecResult* out)
    {
        ...
        RETURN_NOT_OK(OutputAdapter<OutType>::Write(
            ctx,
            out->array_span(),
            [&]() -> OutValue { return Op::template Call<OutValue, Arg0Value>(ctx, arg0_it(), &st); }));
        return st;
    }
};
...
struct ScalarUnaryNotNullStateful
{
    ...
    struct ArrayExec
    {
        static Status Exec(const ThisType& functor, KernelContext* ctx, const ExecSpan& batch, ExecResult* out)
        {
    ...
    {
        static Status Exec(const ThisType& functor, KernelContext* ctx, const ArraySpan& arg0, ExecResult* out)
        {
            Status st = Status::OK();
            auto out_data = out->array_span()->GetValues<OutValue>(1);
            VisitArrayValuesInline<Arg0Type>(
                arg0,
                [&](Arg0Value v) { *out_data++ = functor.op.template Call<OutValue, Arg0Value>(ctx, v, &st); },
                [&]()
                {
                    // null
                    *out_data++ = OutValue{};
    ...
    struct ArrayExec<Type, enable_if_base_binary<Type>>
    {
        static Status Exec(const ThisType& functor, KernelContext* ctx, const ArraySpan& arg0, ExecResult* out)
        {
            ...
            RETURN_NOT_OK(VisitArrayValuesInline<Arg0Type>(
                arg0,
                [&](Arg0Value v) { return builder.Append(functor.op.Call(ctx, v, &st)); },
    ...
    struct ArrayExec<Type, enable_if_t<is_boolean_type<Type>::value>>
    {
        static Status Exec(const ThisType& functor, KernelContext* ctx, const ArraySpan& arg0, ExecResult* out)
        {
            ...
            FirstTimeBitmapWriter out_writer(out_arr->buffers[1].data, out_arr->offset, out_arr->length);
            VisitArrayValuesInline<Arg0Type>(
                arg0,
                [&](Arg0Value v)
                {
...
struct ScalarBinary
{
    ...
    static Status ArrayArray(KernelContext* ctx, const ArraySpan& arg0, const ArraySpan& arg1, ExecResult* out)
    {
        Status st = Status::OK();
        ArrayIterator<Arg0Type> arg0_it(arg0);
        ...
        RETURN_NOT_OK(OutputAdapter<OutType>::Write(
            ...
            out->array_span(),
            [&]() -> OutValue
            { return Op::template Call<OutValue, Arg0Value, Arg1Value>(ctx, arg0_it(), arg1_it(), &st); }));
        return st;
    }

    static Status ArrayScalar(KernelContext* ctx, const ArraySpan& arg0, const Scalar& arg1, ExecResult* out)
    {
        Status st = Status::OK();
        ArrayIterator<Arg0Type> arg0_it(arg0);
        ...
        RETURN_NOT_OK(OutputAdapter<OutType>::Write(
            ...
            out->array_span(),
            [&]() -> OutValue
            { return Op::template Call<OutValue, Arg0Value, Arg1Value>(ctx, arg0_it(), arg1_val, &st); }));
        return st;
    }

    static Status ScalarArray(KernelContext* ctx, const Scalar& arg0, const ArraySpan& arg1, ExecResult* out)
    {
        Status st = Status::OK();
        ...
        RETURN_NOT_OK(OutputAdapter<OutType>::Write(
            ...
            out->array_span(),
            [&]() -> OutValue
            { return Op::template Call<OutValue, Arg0Value, Arg1Value>(ctx, arg0_val, arg1_it(), &st); }));
        return st;
    }
...
struct ScalarBinaryNotNullStateful
{
    ...
    Status ArrayArray(KernelContext* ctx, const ArraySpan& arg0, const ArraySpan& arg1, ExecResult* out)
    {
        Status st = Status::OK();
    ...
    Status ArrayScalar(KernelContext* ctx, const ArraySpan& arg0, const Scalar& arg1, ExecResult* out)
    {
        Status st = Status::OK();
        ...
        if (arg1.is_valid)
        {
            ...
            VisitArrayValuesInline<Arg0Type>(
                arg0,
                [&](Arg0Value u)
                { writer.Write(op.template Call<OutValue, Arg0Value, Arg1Value>(ctx, u, arg1_val, &st)); },
                [&]() { writer.WriteNull(); });
        }
        else
        {
    ...
    Status ScalarArray(KernelContext* ctx, const Scalar& arg0, const ArraySpan& arg1, ExecResult* out)
    {
        Status st = Status::OK();
        ArraySpan* out_span = out->array_span();
        OutputArrayWriter<OutType> writer(out_span);
        if (arg0.is_valid)
        {
            ...
            VisitArrayValuesInline<Arg1Type>(
                arg1,
                [&](Arg1Value v)
                { writer.Write(op.template Call<OutValue, Arg0Value, Arg1Value>(ctx, arg0_val, v, &st)); },
                [&]() { writer.WriteNull(); });
        }
        else
        {
...
template<typename OutType, typename ArgType, typename Op>
using ScalarBinaryNotNullStatefulEqualTypes = ScalarBinaryNotNullStateful<OutType, ArgType, ArgType, Op>;
...
template<template<typename...> class Generator, typename Type0, typename KernelType = ArrayKernelExec, typename... Args>
...
template<typename ReturnType, template<typename... Args> class Generator, typename... Args>
...
//
// See "Numeric" above for description of the generator functor
template<template<typename...> class Generator, typename KernelType = ArrayKernelExec, typename... Args>
...
ARROW_EXPORT
bool CommonTemporalResolution(const TypeHolder* begin, size_t count, TimeUnit::type* finest_unit);
