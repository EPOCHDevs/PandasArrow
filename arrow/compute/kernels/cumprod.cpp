    ...
    Status Accumulate(const ArraySpan& input)
    {
        ...
        if (skip_nulls || (input.GetNullCount() == 0 && !encountered_null))
        {
            VisitArrayValuesInline<ArgType>(
                ...
                [&](ArgValue v)
                {
                    current_value = Op::template Call<OutValue, ArgValue, ArgValue>(ctx, v, current_value, &st);
...
const FunctionDoc cumulative_product_doc{ "Compute the cumulative product over a numeric input",
...
void MakeVectorCumulativeFunction(FunctionRegistry* registry, const std::string func_name, const FunctionDoc doc)
{
    static const OptionsType kDefaultOptions = OptionsType::Defaults();
    auto func = std::make_shared<VectorFunction>(func_name, Arity::Unary(), doc, &kDefaultOptions);
    ...
    for (const auto& ty : types)
    {
        ...
        kernel.exec = ArithmeticExecFromOp<CumulativeKernel, Op, ArrayKernelExec, OptionsType>(ty);
        kernel.exec_chunked =
            ArithmeticExecFromOp<CumulativeKernelChunked, Op, VectorKernel::ChunkedExec, OptionsType>(ty);
...
void RegisterVectorCumulativeProduct(FunctionRegistry* registry)
{
    ...
    MakeVectorCumulativeFunction<MultiplyChecked, CumulativeSumOptions>(
        ...
        cumulative_product_checked_doc);
}

} // namespace internal

CumulativeProductOptions::CumulativeProductOptions(double start, bool skip_nulls, bool check_overflow)
    : CumulativeProductOptions(std::make_shared<DoubleScalar>(start), skip_nulls, check_overflow)
{
}

CumulativeProductOptions::CumulativeProductOptions(std::shared_ptr<Scalar> start, bool skip_nulls, bool check_overflow)
    : FunctionOptions({}), start(std::move(start)), skip_nulls(skip_nulls), check_overflow(check_overflow)
{
}

constexpr char CumulativeProductOptions::kTypeName[];

Result<Datum> CumulativeProduct(const Datum& values, const CumulativeProductOptions& options, ExecContext* ctx)
{
